...
if (empty($_POST['gender']) || empty($_POST['yog']) || empty($_POST['class_code']) || empty($_POST['my_location']) || empty($_POST['athlete']) || empty($_POST['smokingHabits']) || empty($_POST['drinkingHabits'])  || empty($_POST['cleaningHabits']) || empty($_POST['studyHabits'])) {
$filterErr="true";
echo '<font id="error">Please select at least one option for each filter</font>';
}else{
$Gender = $_POST['gender'];
...
$sql1 = 'SELECT profile.* FROM (profile) INNER JOIN (SELECT User FROM locationform Where location IN('.$LocationChoice.') GROUP BY User) locationform ON locationform.User = profile.Username WHERE GENDER IN ('.$GenderChoice.') AND YOGID IN ('.$Year.') AND ClassCode IN ('.$Class.') AND Athlete IN ('.$AthleteChoice.') AND Smoker IN ('.$Smoker.') AND Drinker IN ('.$Drinker.') AND clean IN ('.$Clean.') AND studyHours IN ('.$Study.')';
